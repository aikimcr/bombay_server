...
const parseSQLError = function (req, SQLmessage) {
    ...
    const [, errorType, errorName, fieldList] = matched;
    const fieldsByTable = {};
...
exports.validateForeignKey = async function (tableName, idValue) {
    ...
    return await table.fetchById(idValue)
        .then(model => {
            if (!model) return Promise.reject(new Error(`Invalid ${tableName} id specified: ${idValue}`));
            return model;
        })
        .catch(() => {
            return Promise.reject(new Error(`Invalid ${tableName} id specified: ${idValue}`));
        });
};
